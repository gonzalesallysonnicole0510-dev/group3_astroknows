...
class _QuizGame_NeptuneState extends State<QuizGame_Neptune> {
  ...
  Widget buildLives() {
    return Row(
      children: List.generate(lives, (index) => Icon(Icons.favorite, color: Colors.pink[100])),
    );
  }
  ...
  String feedback = '';
  final List<Map<String, dynamic>> quiz = List.generate(10, (i) {
    return {
      'question': 'Neptune Question ${i + 1}',
      'answers': i % 3 == 0
          ? ['yes', 'x', 'x']
          : i % 3 == 1
              ? ['x', 'yes', 'x']
  ...
  void startQuizGame() {
    gameTimer = Timer.periodic(const Duration(milliseconds: 50), (timer) {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        setState(() => asteroidY[i] += asteroidFloat[i] == direction.UP ? -0.05 : 0.05);
      }
    });
  }
  ...
  void fireLaser() {
    ...
    Timer.periodic(const Duration(milliseconds: 10), (timer) {
      ...
      for (int i = 0; i < 3; i++) {
        if (asteroidY[i] > heightToCoordinate(laserHeight) && (asteroidX[i] - laserX).abs() < 0.35) {
          asteroidY[i] = -50;
          checkAnswer(i);
        }
      }
